...
const CourseCards = ({ searchTerm, subjectFilter }) => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`https://e-learning-server-puce.vercel.app/api/courses`);
        ...
        <Card key={course?._id} onClick={() => handleCardClick(course?._id)}>
          ...
          <CardFooter>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <ProfileImage src={user?.profileImage ? `https://e-learning-server-puce.vercel.app/uploads/${user.profileImage}` : noProfile} alt={user?.username} />
